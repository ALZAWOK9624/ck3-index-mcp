...
- The per-file decision list moved out of `transaction.json` into an appended `decisions.jsonl`, with a small `progress.json` projection for live progress. Planning and building rewrote and fsynced the entire manifest once per file, which is quadratic in the file count; on a large Mod the build phase alone could write orders of magnitude more than the project's own size. The manifest is now written at phase boundaries only. Transaction schema version 2 → 3; existing transactions must be re-planned.
- Promotion and rollback mark the main index stale **before** the first directory rename and refuse to proceed if that fails. Previously the SQLite cache kept reporting `scan_status=ready` while describing the replaced project, so a GUI query would build a cache key from the old file hashes and then read the new file from disk. Every index-backed MCP tool now returns `INDEX_STALE` with the reason and the required recovery until a full refresh republishes the cache.
- `output_dir` placement is proved rather than assumed. `filepath.VolumeName` identifies a drive letter on Windows but is empty for every absolute path on Linux and macOS, so two different mount points compared equal and promotion could move the project to its backup and then fail the second rename with `EXDEV`. Planning now accepts a sibling of the formal project outright and otherwise probes a real cross-parent rename; promotion re-checks it immediately before the first rename.
- Automatic Jomini object merge is restricted to an explicit domain allowlist (events, scripted effects/triggers/modifiers, cultures, traditions, religions/faiths, landed titles, and character/province/title history). Opening it to every `.txt`/`.gui`/`.asset`/`.settings`/`.mod` file was too wide: those formats repeat generic top-level keys and carry their real identity in a child field. A path outside the list becomes an ordinary review conflict.
- Semantic identity now distinguishes *absent* from *ambiguous*. A project object whose ID appeared more than once in the old or new upstream previously looked absent, and the merge appended the project copy beside the existing ones; it now always stops for review. GUI declarations get their own identities (`gui:type:*`, `gui:template:*`, `gui:root:<kind>:<name>`), and declarations with no unambiguous identity — anonymous blocks, unnamed root widgets — block the file from being merged instead of colliding on a generic key.
- New optional profile keys. `validation_stack` describes the real ordered load stack for a dependency that overrides the new upstream, instead of `validation_sources` silently placing every auxiliary source below the target. `preserve_paths` names top-level project entries that are external state (VCS metadata, caches, logs): they are excluded from the overlay inventory, the migration copy, and every transaction fingerprint, and they move with the project directory across promotion and rollback rather than being copied. Omitting both keys keeps the previous behaviour exactly.

### Index and asset robustness

- `ck3_refresh` now fails closed when `ck3-index.toml` cannot be reloaded. The server parses the configuration once at startup and scans from that copy, so a deleted, half-written, or invalid file would previously have scanned the previously configured roots and published a fresh `ready` generation. Drift detection also compares an internal fingerprint over full canonical paths, database, and engine-log locations rather than the redacted display identity, whose trailing path segments two unrelated roots can share.
- TGA decoding enforces a pixel budget derived from the active map before allocating. The 16-bit dimension fields let a corrupt file claim 65535×65535 and request a 17 GB frame buffer. Right-to-left image descriptors are rejected rather than read mirrored, and RLE decoding and the full-map scan honour cancellation.

### GUI

- `model_samples` no longer writes into the shared GUI resolution cache. `prepareGUIModelSamples` takes and returns a value and expands rows on a private deep copy, so a request without samples cannot inherit rows injected by an earlier one and two concurrent previews cannot race on the same slices.
- The diagnostic PNG is encoded exactly once, and only for formats that carry one. Scene construction rendered it before localization, runtime facts, scenario values, and model rows were applied, and `refreshGUIPreviewPNG` then rendered it again; `format=html` paid for two encodes and discarded both. `BuildGUIPreviewScene` now stops at the laid-out scene.
- GUI diagnostics are ranked by severity and then truncated, using an explicit rank instead of a string comparison (which ordered error → info → warning). A run of leading informational findings could previously evict every error from a bounded response.
