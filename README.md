...
面向《十字军之王 III》本地模组修改辅助和 QQ bot 问答的语义索引与 MCP 服务。它把 Mod、游戏本体和可选参考来源按优先级合并索引：问答使用允许公开的定义、本地化和来源证据快速回答；本地开发使用编辑准备、检验、增量刷新和诊断完成授权的修改。技能按任务意图区分两条流程，QQ 问答不会因此取得未公开工程的读取或修改权限。

本仓库不包含游戏文件、Mod 内容、索引缓存或个人配置。

插件还注册独立的 `ck3_check` 服务：模型生成或修正代码后，先提交虚拟文件校验、修复报错并重新检查，再输出最后通过检查的版本。这个服务无需数据库，也不读取拟议路径对应的本机文件；语法和已覆盖静态规则通过不代表跨文件引用或实机行为已验证。

无需建立索引即可检查拟议脚本：新增 `ck3-check` / `ck3-index check` 与独立的 QQ bot MCP 服务，见 [无数据库 DSL 检查与原版语料审查](docs/STANDALONE_DSL_CHECK.md)。

## 适合做什么

- 为 QQ bot 提供有来源的机制、名称、本地化和已索引设定问答；复用已有证据，按需检索，避免把普通问题变成全工程审查。
