...
var chineseFieldDescriptions = map[string]string{
	...
	"One-based evidence page. Pages are stable only for the published scan generation returned with the tool result.":                         "从 1 开始的证据页码；仅在工具结果返回的已发布扫描 generation 未变化时页面顺序稳定。",
	...
	"Use public to return only configured non-private source evidence; aggregate counts and operations requiring private workspace evidence are unavailable.": "设为 public 时仅返回已配置的非私有来源证据；聚合计数和需要私有工作区证据的操作不可用。",
