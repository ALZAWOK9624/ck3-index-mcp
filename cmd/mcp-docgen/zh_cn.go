...
var chineseToolTexts = map[string]chineseToolText{
	...
	"map_split_province": {
		Title:       "规划省份切割",
		Description: "按调用方给定的种子像素，以地形加权生长把一个已索引省份切开，使边界沿山脊落下而不是切出直线，再规划 CK3 保持一致所需的 definition.csv、history 与领地头衔改动。分配不冲突的省份 ID 与颜色，无法证明唯一性时给出阻塞项。只返回可审查的方案，不写入任何文件。",
	},
	"map_apply_split": {
		Title:       "应用省份切割",
		Description: "重放一次省份切割，把重新着色的 provinces.png 写入服务器产物目录，并附上必须同时应用的 definition.csv、history 与领地头衔改动。写入前逐像素比对索引记录的省份颜色，索引过期或方案存在阻塞项时拒绝写入。Mod 本身不会被修改。",
	},
	"map_terrain_edit": {
		Title:       "编辑 CK3 物理地貌",
		Description: "按顺序合成点、折线或多边形地貌图层，下切大河河谷，或只替换指定窗口内的小河并保留窗口外内容。confirm=false 仅返回内存预览，confirm=true 才创建带哈希和父级校验的不可变产物。只输出原始高度图与必要的 rivers.png，绝不修改 Mod，并明确要求经 CK3 地图编辑器重新打包。",
	},
}
