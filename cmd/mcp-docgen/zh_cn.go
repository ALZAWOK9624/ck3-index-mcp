...
var chineseToolTexts = map[string]chineseToolText{
	...
	"ck3_health": {
		Title:       "检查 CK3 索引健康状态",
		Description: "检查数据库、结构、索引与 MCP 注册是否可信，并报告 SQLite 读取连接、已加载/退役连接池、进程总缓存预算及限额、当前重型/栅格任务和估算任务内存。配置与来源根可辨识，数据库绝对路径保持隐藏。",
	},
	"ck3_database": {
		Title:       "选择 CK3 索引数据库",
		Description: "列出管理员配置的 SQLite 索引、报告当前数据库，或在不重启 MCP 的情况下按名称热切换后续调用。运行中的调用继续持有原数据库租约及连接池预算；候选库超出进程总预算时保持当前数据库不变。调用方不能提交文件路径。",
	},
	"ck3_package": {
		Title:       "打包 CK3 Mod",
	...
	"map_split_province": {
		Title:       "规划省份切割",
		Description: "按调用方给定的种子像素，以地形加权生长切分一个已索引省份，并发布绑定当前扫描代次、provinces.png 与 definition.csv 哈希的不可变方案产物。规划 definition.csv、history 与领地头衔改动，无法证明安全时给出阻塞项；Mod 本身不会被修改。",
	},
	"map_apply_split": {
		Title:       "应用省份切割",
		Description: "加载此前审查过的精确不可变切分几何，以原子方式发布重新着色的 provinces.png，并附上必须同时应用的 definition.csv、history 与领地头衔改动。支持可选 request_key 恢复同一次提交；方案哈希、扫描代次或地图源文件发生漂移时拒绝写入，Mod 本身不会被修改。",
	},
	"map_terrain_edit": {
		Title:       "编辑 CK3 物理地貌",
		Description: "按顺序合成点、折线或多边形地貌图层，下切大河河谷，或只替换指定窗口内的小河并保留窗口外内容。confirm=false 仅返回内存预览，confirm=true 才创建带哈希、父级校验和可选幂等键的不可变产物。只输出原始高度图与必要的 rivers.png，绝不修改 Mod，并明确要求经 CK3 地图编辑器重新打包。",
	},
	"map_artifact": {
		Title:       "检查地图产物",
		Description: "列出、查询状态或完整检查已提交的地貌、省份切割方案与切割栅格产物。验证不可变 manifest 和输出哈希，使调用方在超时后能找回产物 ID，而不会重复生成文件。",
	},
}

var chineseFieldDescriptions = map[string]string{
	"Text presentation: compact (default) uses columns/rows tables. Merge shared fields into each row; null cells mean absent fields; integer path cells index the zero-based paths dictionary. json mirrors structuredContent for legacy text parsers. structuredContent is unchanged in either mode.": "正文格式：默认 compact 使用 columns/rows 表格；各行继承 shared 字段，null 单元格表示字段缺省，整数 path 单元格引用从零开始的 paths 字典。json 为旧版正文解析器保留与 structuredContent 相同的 JSON；两种模式的 structuredContent 均不变。",
	...
	"Save view. card is the default and identifies the save; compatibility lists the content the save declares; audit checks every id the save carries against the indexed sources and reports the played character; character profiles one character from the gamestate, the played one unless another save id is named; timeline collects the save's dated events; document reads any path in the gamestate.": "存档视图。card 为默认值，用于识别存档；compatibility 列出存档声明的内容；audit 把存档携带的每个 ID 与已索引来源核对，并报告存档记录的游玩角色；character 从 gamestate 中提取单个角色的档案，未另行指定存档 ID 时取该游玩角色；timeline 汇总存档记录的带日期事件；document 读取 gamestate 中的任意路径。",
	"Save id of a character. operation=character profiles the save's own played character when this is omitted; operation=timeline keeps only events naming this character.":                                                                                                                                                                                                                                    "角色的存档 ID。operation=character 省略时提取存档自身记录的游玩角色；operation=timeline 只保留涉及该角色的事件。",
	"Dotted path inside the gamestate for operation=document, such as landed_titles.landed_titles.4501 or character_memory_manager.database.0. Empty lists the top level. Use key[n] to pick among repeated keys.":                                                                                                                                                                                              "operation=document 使用的 gamestate 内点分路径，例如 landed_titles.landed_titles.4501 或 character_memory_manager.database.0。留空则列出顶层。重复键用 key[n] 指定第几个。",
	"How many levels below the path operation=document materialises. 0 lists the children only, which is always reported and is the cheapest useful answer.":                                                                                                                                                                                                                                                    "operation=document 在该路径下展开几层。0 只列出直接子节点——这一项始终返回，也是最省的有效答案。",
	"How many events operation=timeline returns, most recent first-dated last. The totals it reports are of every matching event, not just these.":                                                                                                                                                                                                                                                              "operation=timeline 返回多少个事件，取最近的若干个、按时间正序排列。它报告的总数统计的是全部命中事件，不只是返回的这些。",
	"Which event kinds operation=timeline collects. Omit for every kind. A vassal contract or court appointment is dated at the campaign's start and will crowd out later events unless narrowed.":                                                                                                                                                                                                              "operation=timeline 收集哪些事件种类，省略则全收。封臣契约和宫廷任命的日期多在开局当天，不筛选会把后来的事件挤掉。",
	...
	"Indexed province id to divide.":                            "要切割的已索引省份 ID。",
	"Growth origins, one per resulting province. The largest resulting part keeps the original province id and colour.":                                                                                           "生长起点，每个结果省份一个。面积最大的那块保留原省份 ID 与颜色。",
	"How strongly terrain deflects boundaries. 0 grows by distance alone and yields geometric edges; higher values bend boundaries toward steep ground. Requires an indexed elevation raster to have any effect.": "地形对边界的偏转强度。0 表示只按距离生长，边界会是几何直线；数值越大，边界越贴向陡坡。需要已索引的高程栅格才会生效。",
	"Include the definition.csv rows for the new provinces.":                                                                                                "包含新省份的 definition.csv 行。",
	"Include history/provinces entries inheriting the parent's culture, faith, and holding.":                                                                "包含 history/provinces 条目，继承母省的文化、信仰与地产。",
	"Include barony skeletons for the owning county.":                                                                                                       "包含所属伯爵领的男爵领骨架。",
	"Must match the value planned with map_split_province, or the geometry written will not be the geometry reviewed.":                                      "必须与 map_split_province 规划时使用的值一致，否则写入的几何与审查过的几何不是同一个。",
	"Must be true. Acknowledges that a new provinces.png will be written to the server artifact area.":                                                      "必须为 true，用以确认将向服务器产物目录写入新的 provinces.png。",
	"False validates and returns an in-memory PNG preview; true creates a new immutable artifact.":                                                          "false 仅校验并返回内存中的 PNG 预览；true 才创建新的不可变产物。",
	"Optional idempotency key. Retrying the same publication with the same key returns its committed artifact; reusing it for different input is rejected.": "可选幂等键；用同一键重试同一发布会返回已提交产物，把该键用于不同输入则会被拒绝。",
	"Artifact recovery operation.":                                                                                                       "产物恢复操作。",
	"Controlled map artifact id. Required for status or inspect.":                                                                        "受控地图产物 ID；status 或 inspect 操作必须提供。",
	"Maximum artifacts returned by list.":                                                                                                "list 操作最多返回的产物数量。",
	"Which physical-terrain stage to run.":                                                                                               "要运行的物理地貌阶段。",
	"Optional verified parent map-edit artifact id. Omit to read the current effective map.":                                             "可选的已验证父级地图编辑产物 ID；省略时读取当前生效地图。",
	"Ordered terrain layers. Each layer works from the result of the previous layer.":                                                    "有序地貌图层；每层都在前一层结果上继续塑形。",
	"Heightmap pixel coordinates, with the top-left pixel as the origin.":                                                                "高度图像素坐标，以左上角像素为原点。",
	"Geometry type. Point needs one coordinate, polyline at least two, and a simple non-self-intersecting polygon at least three.":       "几何类型：点需要一个坐标，折线至少两个，简单非自交多边形至少三个。",
	"Unique stable id within this ordered compose request.":                                                                              "本次有序 compose 请求中唯一且稳定的图层 ID。",
	"Physical landform profile.":                                                                                                         "物理地貌剖面类型。",
	"Full influence width in heightmap pixels.":                                                                                          "以高度图像素计的完整影响宽度。",
	"Smooth falloff beyond the geometry influence.":                                                                                      "几何影响范围之外的平滑羽化宽度。",
	"Province-defined target domain. Defaults by landform kind; it is never inferred from elevation.":                                    "由省份定义决定的目标域；默认值按地貌类型选择，绝不从高程猜测。",
	"Normalized 0..1 edit magnitude or blend strength.":                                                                                  "归一化到 0..1 的编辑幅度或混合强度。",
	"Mix between the analytic profile and deterministic fractal detail.":                                                                 "解析剖面与确定性分形细节之间的混合比例。",
	"Noise frequency scale.":                                                                                                             "噪声频率缩放。",
	"Normalized target plane required by plain, continental_shelf, continental_slope, and abyssal_plain.":                                "plain、continental_shelf、continental_slope 与 abyssal_plain 所需的归一化目标平面。",
	"Deterministic noise seed.":                                                                                                          "确定性噪声种子。",
	"Settings for operation=large_rivers. Major rivers are carved into the raw heightmap and leave small-river hydrology stale.":         "operation=large_rivers 的设置；大河会下切进原始高度图，并把小河水系标记为过期。",
	"Settings for operation=small_rivers. Only the selected window is replaced; palette indices outside it are inherited byte-for-byte.": "operation=small_rivers 的设置；仅替换所选窗口，窗口外调色板索引逐字节继承。",
	"Normalized depth of the largest channels.":                                                                                          "最大河道的归一化下切深度。",
	"Normalized water outlet level.":                                                                                                     "归一化的水体出口高度。",
	"Normalized shallow bed depth.":                                                                                                      "归一化的浅河床深度。",
	"Normalized water level.":                                                                                                            "归一化水位。",
	"Hydraulic erosion settings. Omit to skip erosion, which also skips the drainage that ties separately placed landforms into one landscape.":                                                            "水力侵蚀设置。省略则跳过侵蚀，同时也就没有把分开放置的地貌连成同一片地景的水系。",
	"Pixel window the generator may read and change. Erosion and drainage routing both cost time proportional to the area covered, so bound them to the ground you care about. Defaults to the whole map.": "生成器可读写的像素窗口。侵蚀与汇流计算的耗时都与覆盖面积成正比，因此应把范围限制在关心的地面上。默认为整幅地图。",
	"Return a companion lookup plate alongside a basemap so a page can resolve the entity under the pointer exactly. Roughly doubles the returned image payload.":                                          "随底图额外返回一张查找图，使网页能精确判定指针下的实体；返回的图像载荷大约翻倍。",
	...
	"One-based evidence page, stable only within the returned scan generation.":                                                               "从 1 开始的证据页码；仅在返回的扫描 generation 内稳定。",
	"Maximum encoded MCP tool-result size in bytes. Use limit/page to reduce semantic evidence first; this is a hard response safety budget.": "MCP 工具结果编码后的最大字节数；应先用 limit/page 缩小语义证据，此项是硬响应安全上限。",
	...
	"Optional substring filter over texture name or kind for operation=assets.":                                        "operation=assets 时对贴图名称或种类的可选子串筛选。",
	...
	"Refresh operation. status is the default and does not change the index; files incrementally refreshes named project files; full requests an explicit full scan.":                        "刷新操作；status 为默认值且不改变索引，files 增量刷新指定工程文件，full 明确请求完整扫描。",
	"Refresh operation. status is the default and does not change the index; files incrementally refreshes named project files; full stages and atomically publishes an explicit full scan.": "刷新操作；status 为默认值且不改变索引，files 增量刷新指定工程文件，full 会先暂存完整扫描并原子发布。",
	"Source-root-relative project paths for operation=files. Absolute paths and parent traversal are rejected.":                                                                              "operation=files 使用的工程源根相对路径；拒绝绝对路径和上级目录穿越。",
	...
	"Optional caller-provided example results for exact GUI expressions. Values are labeled provided, never observed, and unmatched expressions are reported. Image-bearing texture, video, and coat_of_arms_mask samples must name an indexed source-root-relative gfx asset.": "可选的调用方精确 GUI 表达式样例结果；数值明确标记为 provided 而非游戏观察事实，未命中表达式会被报告。带图像的 texture、video 与 coat_of_arms_mask 样例必须指向已索引的 gfx 源根相对资源。",
	...
	"Provided text; indexed source-root-relative gfx image for texture, video, or coat_of_arms_mask; two finite numbers for vector fields; or true/false for visible and enabled.": "调用方提供的文本；texture、video 或 coat_of_arms_mask 使用已索引的 gfx 源根相对图像；向量字段使用两个有限数值；visible 与 enabled 使用 true/false。",
	"Row-local GUI property whose exact expression result is sampled.":                                                                                                                 "要提供精确表达式样例结果的行内 GUI 属性。",
	...
	"Provided row text; indexed source-root-relative gfx image for texture, video, or coat_of_arms_mask; two finite numbers for vector fields; or true/false for visible and enabled.": "调用方提供的行文本；texture、video 或 coat_of_arms_mask 使用已索引的 gfx 源根相对图像；向量字段使用两个有限数值；visible 与 enabled 使用 true/false。",
}
