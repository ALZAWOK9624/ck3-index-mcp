...
插件同时注册索引与独立检查服务。检查服务由单独启动脚本调用 `ck3-index check --serve`，不读取插件索引设置、SQL 配置或 GIS 环境；索引不可用时仍可校验提交文本。

## 返回值的正确解读

`passed` 只表示本次静态检查没有 error；不是 CK3 官方编译器的认可。
MCP 只返回一份 `structuredContent`，`content` 为空。每个文件的 `content_sha256` 是提交文本的 UTF-8 SHA-256；修改代码后必须重新检查，回执不能沿用到另一版文本。
每个文件都报告覆盖情况：
