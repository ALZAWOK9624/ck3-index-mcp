...
func (db *DB) Health(ctx context.Context) (HealthReport, error) {
	...
	report := HealthReport{
		...
		WALFiles: []HealthFile{
			{Path: dbPath + "-wal", Exists: fileExists(dbPath + "-wal"), SizeMB: fileSizeMB(dbPath + "-wal")},
			{Path: dbPath + "-shm", Exists: fileExists(dbPath + "-shm"), SizeMB: fileSizeMB(dbPath + "-shm")},
		},
		MCPConfigured: codexMCPConfigured(),
	}
	if err := db.sql.QueryRowContext(ctx, `SELECT count(*) FROM search_fts WHERE search_fts MATCH 'ck3indexhealthtoken'`).Scan(new(int)); err == nil {
