...
// Tiger list and the live on_actions.log evidence captured in the cache. It
...
type OnActionRuleAudit struct {
	...
	TigerCount            int                 `json:"tiger_count"`
	EngineOnlyCount       int                 `json:"engine_only_count"`
	TigerOnlyCount        int                 `json:"tiger_only_count"`
	EngineOnly            []OnActionRuleDelta `json:"engine_only,omitempty"`
	TigerOnly             []OnActionRuleDelta `json:"tiger_only,omitempty"`
...
// script-rule drift: on_actions.log is broader and newer than the generated
// static table. A missing engine log cache is reported as unavailable rather
// than turning every static entry into a false discrepancy.
func (db *DB) AuditOnActionRules(ctx context.Context, limit int) (OnActionRuleAudit, error) {
	...
	report := OnActionRuleAudit{
		...
		TigerCount:            len(tigerOnActions),
		Guidance: []string{
			"Engine-only and Tiger-only entries are review evidence, not automatic validation changes.",
	...
	for name := range live {
		if _, ok := tigerOnActions[name]; !ok {
			engineOnlyNames = append(engineOnlyNames, name)
		}
	}
	tigerOnlyNames := make([]string, 0)
	for name := range tigerOnActions {
		if _, ok := live[name]; !ok {
			tigerOnlyNames = append(tigerOnlyNames, name)
		}
	}
	sort.Strings(engineOnlyNames)
	sort.Strings(tigerOnlyNames)
	report.EngineOnlyCount = len(engineOnlyNames)
	report.TigerOnlyCount = len(tigerOnlyNames)
	...
	for _, name := range tigerOnlyNames {
		if len(report.TigerOnly) >= limit {
			report.Truncated = true
			break
		}
		report.TigerOnly = append(report.TigerOnly, OnActionRuleDelta{
			Name: name, Classification: "tiger_only", RuleSource: "tiger_fallback", Confidence: "medium",
		})
	}
	return report, nil
}
