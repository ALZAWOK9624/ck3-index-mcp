...
func (db *DB) LLMSearch(ctx context.Context, opts SearchOptions) (LLMResult, error) {
	...
	// The near-miss pass runs only after every ordinary searcher, the FTS pass,
	// the script-text pass, and the case-sensitive contains fallback have all
	...
	// real searches ran public -- but the caller-facing guidance below is
	// delivered under both visibilities, so recovery advice does not depend on
