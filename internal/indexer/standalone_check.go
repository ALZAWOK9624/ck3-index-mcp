...
func (c *StandaloneChecker) Check(ctx context.Context, req CheckRequest) (CheckResult, error) {
	...
	result := CheckResult{Version: StandaloneCheckVersion, Status: "checked", Passed: true,
		...
		Guidance: "Pass means no errors in the reported static coverage. Unknown commands, dynamic scopes, object/localization/resource references, load order and game execution are not certified. Use indexed ck3_review and in-game testing for those checks."}
	if c.rules != nil {
...
func (c *StandaloneChecker) checkText(rel, content string, syntaxOnly bool, role SourceRole) (CheckFileResult, script.File) {
	...
	r := CheckFileResult{Path: rel, Dialect: dialect, Diagnostics: []Diagnostic{},
