...
func workspaceCapabilities(ctx context.Context, runtime *Runtime, requestedDomain string) (map[string]any, error) {
	...
	items := []workspaceCapability{
		...
		{ID: "diagnostic_baselines", Domain: "diagnostics", Source: "sqlite_index", Tools: []string{"ck3_diagnostic_baseline"}, Modes: []string{"save", "list", "clear"}, Inputs: []string{"a baseline name"}, Outputs: []string{"the recorded baselines and what each holds"}, Maturity: "stable", RequiresReadyIndex: true, SideEffect: "records the caller's accepted findings in the index cache", Cost: "low", Profile: "default", Available: indexAvailable, Reason: indexReason},
		...
		{ID: "coat_of_arms", Domain: "coat_of_arms", Source: "sqlite_index", Tools: []string{"ck3_coat_of_arms"}, Modes: []string{"inspect", "render", "assets"}, Inputs: []string{"exact coat of arms id, or an asset filter"}, Outputs: []string{"resolved colours and texture references, and a rendered field PNG"}, Maturity: "stable", RequiresReadyIndex: true, SideEffect: "none", Cost: "medium", Profile: "default", Available: indexAvailable, Reason: indexReason},
