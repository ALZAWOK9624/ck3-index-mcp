...
func standardizeCanonicalToolDescriptions(definitions []ToolDefinition) []ToolDefinition {
	boundaries := map[string]catalogToolBoundary{
		...
		"ck3_save": {
			...
			Input:    "a save file inside a configured save root, plus card, compatibility, audit, or character",
			Returns:  "save identity and declared content, or the ids a save carries that no indexed source defines, or one character's attributes, traits, house, and titles",
