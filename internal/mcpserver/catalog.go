...
func standardizeCanonicalToolDescriptions(definitions []ToolDefinition) []ToolDefinition {
	...
	// Only the disambiguating clauses are prepended. Input and Returns restated
	// what the input schema's own property descriptions and the tool's base
	// description already say, which is catalog weight that buys no extra
	// selection accuracy; When/DoNotUse/Unlike are what separate near-neighbours.
	for i := range definitions {
		...
		definitions[i].Description = "Use when " + boundary.When + ". Do not use " + boundary.DoNotUse + ". Unlike " + boundary.Unlike + ". " + definitions[i].Description
	}
	return definitions
}
