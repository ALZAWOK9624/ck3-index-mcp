...
func legacyToolCatalog() []map[string]any {
	...
	mapRenderSchema := map[string]any{
		...
		"properties": map[string]any{
			...
			"style": map[string]any{"type": "string", "enum": []string{"parchment", "standard", "historical_atlas"}}, "layout": map[string]any{"type": "string", "enum": []string{"map_only", "light_frame", "full_atlas", "basemap"}}, "relief_strength": map[string]any{"type": "string", "enum": []string{"none", "subtle", "strong"}}, "label_language": map[string]any{"type": "string", "enum": []string{"chinese", "english", "bilingual"}}, "color_strategy": map[string]any{"type": "string", "enum": []string{"native", "muted", "coordinated"}}, "supersample": map[string]any{"type": "integer", "enum": []int{1, 2}},
